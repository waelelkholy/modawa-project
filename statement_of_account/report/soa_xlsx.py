    ...
    def generate_xlsx_report(self, workbook, report_detail, wizard_data):
        ...
        for customer_id in partner_ids:
            ...
            if raw_data:
                ...
                worksheet.write_string(row, 5, 'Tax Excluded', line_heading_format)
                worksheet.write_string(row, 6, 'Tax Amount', line_heading_format)
                worksheet.write_string(row, 7, 'Currency', line_heading_format)
                worksheet.write_string(row, 8, 'Debit', line_heading_format)
                worksheet.write_string(row, 9, 'Credit', line_heading_format)
                # worksheet.write_string(row, 9, 'Payment Recd', line_heading_format)
                worksheet.write_string(row, 10, 'Balance SAR', line_heading_format)
                row = row + 1
                for i in raw_data:
                    ...
                    worksheet.write_string(row, 5, str(i['inv_amt_untaxed'] or ''), line_data)
                    worksheet.write_string(row, 6, str(i['amount_tax'] or ''), line_data)
                    worksheet.write_string(row, 7, str(i['currency'] or ''), line_data)
                    worksheet.write_number(row, 8, i['debit'], line_data)
                    worksheet.write_number(row, 9, i['credit'], line_data)
                    # worksheet.write_number(row, 9, i['payment_rec'], line_data)
                    worksheet.write_number(row, 10, i['bal'], line_data)
                    row += 1
                ...
                worksheet.write_string(row, 6, 'Total', line_heading_format)
                worksheet.write_string(row, 7, '', line_heading_format)
                worksheet.write_number(row, 8, total_debit, line_heading_format)
                worksheet.write_number(row, 9, total_credit, line_heading_format)
                # worksheet.write_number(row, 9, total_payment_rec, line_heading_format)
                worksheet.write_number(row, 10, total_balance, line_heading_format)
