...
class CashPaymentRegister(models.TransientModel):
    _name = "cash.register.payment"
    _description = "Cash Loan Register Payment"

    def get_journal(self):
        context = self.env.context
        loan = self.env['hr.loan'].browse(context.get('active_id'))
        if not loan.journal_id:
            raise UserError('Kindly define journal on Loan')
        return loan.journal_id.id
    ...
    def action_create(self):
        ...
        if loan.installment_type == 'installment_no':
            per_installment_amount = loan.loan_amount / loan.installment
            if self.amount % per_installment_amount != 0:
                raise UserError('Kindly add amount in the sequence of per installment')
        elif loan.installment_type == 'installment_amount':
            per_installment_amount = loan.installment_amount
            if self.amount % per_installment_amount != 0:
                raise UserError('Kindly add amount in the sequence of per installment')
        ...
        for payment in loan.payment_ids:
            created_payment_list.append(payment.id)
        created_payment_list.append(create_payment.id)
        loan.update({'payment_ids': created_payment_list})
        loan_lines = self.env['hr.loan.line'].search([('loan_id', '=', loan.id), ('status', '=', 'pending')],
                                                   order='date asc', limit=limit)
        loan_lines.update({'status': 'done'})
